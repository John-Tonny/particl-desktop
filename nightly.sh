...
then 
    ...
    cd packages
    mv `ls | grep "vircle-desktop.*linux-x64.zip"` vircle-desktop-linux-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    mv `ls | grep "vircle-desktop.*linux-amd64.deb"` vircle-desktop-linux-amd64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.deb
    cd ..
...
then 
    ...
    cd packages
    mv `ls | grep "vircle-desktop.*mac.zip"` vircle-desktop-mac-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    cd ..
...
#    echo disable > "${WINEPREFIX:-${HOME}/.wine}/.update-timestamp"
    ...
    cd packages
    zip -r vircle-desktop-win-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-unpacked
    cd ..

    DEBUG=electron-builder yarn run travis:win32

    cd packages
    zip -r vircle-desktop-win-ia32-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-ia32-unpacked
    cd ..
...
then 
    ...
    for fn in `ls | grep "vircle-desktop"`; do
        echo "Uploading $fn"
