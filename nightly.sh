...
then 
    ...
    cd packages
    mv `ls | grep "vpub-desktop.*linux-x64.zip"` vpub-desktop-linux-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    mv `ls | grep "vpub-desktop.*linux-amd64.deb"` vpub-desktop-linux-amd64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.deb
    cd ..
...
then 
    ...
    cd packages
    mv `ls | grep "vpub-desktop.*mac.zip"` vpub-desktop-mac-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    cd ..
...
#    echo disable > "${WINEPREFIX:-${HOME}/.wine}/.update-timestamp"
    ...
    cd packages
    zip -r vpub-desktop-win-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-unpacked
    cd ..

    DEBUG=electron-builder yarn run travis:win32

    cd packages
    zip -r vpub-desktop-win-ia32-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-ia32-unpacked
    cd ..
...
then 
    ...
    for fn in `ls | grep "vpub-desktop"`; do
        echo "Uploading $fn"
